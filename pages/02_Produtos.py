...
<div style="margin-bottom:2rem;">
  <div style="font-size:0.68rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#f97316;">PRINT LAB</div>
  <h1 style="margin:0.25rem 0 0.5rem 0;">Produtos</h1>
  <p style="color:#525252;margin:0;font-size:0.95rem;">Catálogo completo com todos os custos por produto.</p>
</div>
...
with tab_imp:
    ...
    if impressoras:
        for imp in impressoras:
            with st.expander(f"**{imp['nome']}** — {imp['consumo_watts']}W · R$ {imp['custo_aquisicao']:,.0f} · {imp['vida_util_horas']:.0f}h"):
                with st.form(f"edit_imp_{imp['id']}"):
                    st.markdown('<span style="font-size:0.68rem;color:#f97316;text-transform:uppercase;letter-spacing:0.1em">Editar</span>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="color:#404040;font-size:0.9rem">Nenhuma impressora cadastrada ainda.</div>', unsafe_allow_html=True)
...
with tab_mat:
    ...
    if materiais:
        ...
        st.markdown('<div style="color:#404040;font-size:0.9rem">Nenhum material cadastrado ainda.</div>', unsafe_allow_html=True)
...
with tab_prod:
    ...
    if produtos:
            ...
            with st.expander(f"**{p['nome']}** — R$ {p['preco_venda']:.2f} · Margem {m:.1f}%"):
                ...
                <div style="margin:0.5rem 0 1.25rem;">
                  <div style="display:flex;align-items:center;gap:0.75rem;">
                    <span style="font-size:0.65rem;color:#404040;text-transform:uppercase;letter-spacing:0.1em">Margem</span>
                ...
                with st.form(f"edit_prod_{p['id']}"):
                    st.markdown('<span style="font-size:0.68rem;color:#f97316;text-transform:uppercase;letter-spacing:0.1em">Editar produto</span>', unsafe_allow_html=True)
                    ...
                    tempo_imp = c2.number_input("Tempo impressão (h)", value=float(p["tempo_impressao_h"]), step=0.5,  key=f"ti_{p['id']}")
                    preco_v   = c3.number_input("Preço venda (R$)",    value=float(p["preco_venda"]),       step=0.5,  key=f"pv_{p['id']}")
                    c1, c2 = st.columns(2)
                    tempo_pos = c1.number_input("Pós-proc. (h)", value=float(p["tempo_pos_proc_h"]), step=0.25, key=f"tp_{p['id']}")
                ...
                # ── Custos adicionais ─────────────────────────────────────────
                st.divider()
                st.markdown('<span style="font-size:0.68rem;color:#737373;text-transform:uppercase;letter-spacing:0.1em">Custos Adicionais</span>', unsafe_allow_html=True)
                ...
                if extras:
                    ...
                    st.markdown('<span style="color:#404040;font-size:0.85rem">Nenhum custo adicional ainda.</span>', unsafe_allow_html=True)

                # Adicionar novo custo adicional
                with st.form(f"add_extra_{p['id']}"):
                    st.markdown('<span style="font-size:0.65rem;color:#525252;text-transform:uppercase;letter-spacing:0.08em">+ Novo custo adicional</span>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="background:#0d0d0d;border:1px dashed #2a2a2a;border-radius:10px;padding:2rem;text-align:center;color:#404040">Nenhum produto cadastrado ainda.</div>', unsafe_allow_html=True)
    ...
    if not impressoras or not materiais:
        ...
        with st.form("form_prod"):
            ...
            tempo_imp = c2.number_input("Tempo impressão (h)", value=2.0,   min_value=0.1, step=0.5)
            preco_v   = c3.number_input("Preço de venda (R$)", value=25.0,  min_value=0.01, step=0.5)
            st.markdown("**Mão de obra**")
            c1, c2 = st.columns(2)
            tempo_pos = c1.number_input("Pós-proc. (h)",     value=0.0, min_value=0.0, step=0.25)
